...
try:
    ...
    if priority_incident_ids:
        ...
        st.session_state["active_incident"] = selected_incident
...
st.header("Operator Activity Summary")

try:

    activity_summary = get_operator_action_summary()
    ...
    with col1:
        st.metric(
            ...
            activity_summary["total_actions"],
        )

    with col2:
        st.metric(
            "Escalation Actions",
            activity_summary["escalation_actions"],
        )

    with col3:
        latest_action = activity_summary["latest_action_at"]

        st.metric(
            "Latest Action",
            latest_action.strftime("%Y-%m-%d %H:%M")
            if latest_action
            else "None",
        )
    ...
    st.error(
        "Unable to load operator activity summary."
    )
...
try:
    ...
    impact = get_business_impact_summary()
    ...
    with col1:
        st.metric(
            "Total Incidents",
            f"{impact['total_incidents']:,}",
        )

    with col2:
        st.metric(
            "Total Prolonged Incidents",
            f"{impact['total_prolonged_incidents']:,}",
        )

    with col3:
        st.metric(
            "High-Risk Incidents",
            f"{impact['high_risk_incidents']:,}",
        )
    ...
    with col1:
        st.metric(
            "High-Risk Prolonged Incidents",
            f"{impact['high_risk_prolonged_incidents']:,}",
        )

    with col2:
        st.metric(
            "High-Risk Prolonged Rate",
            f"{impact['high_risk_prolonged_rate_pct']:.2f}%",
        )

    with col3:
        st.metric(
            "Share of All Prolonged Incidents",
            f"{impact['high_risk_prolonged_share_pct']:.2f}%",
        )

    st.write(
        f"High-risk incidents account for "
        f"**{impact['high_risk_prolonged_incidents']:,} prolonged "
        f"resolutions**, representing "
        f"**{impact['high_risk_prolonged_rate_pct']:.2f}%** of "
        f"high-risk incidents and "
        f"**{impact['high_risk_prolonged_share_pct']:.2f}%** of "
        f"all prolonged incidents."
    )

except Exception as error:

    st.error(
        "Unable to load business impact summary."
    )
...
try:
    ...
    recommendations = get_management_recommendations()
    ...
    for item in recommendations:
        ...
        prolonged_incidents = item["prolonged_incidents"]
        prolonged_rate = item["prolonged_rate_pct"]
        recommendation = item["recommendation"]

        st.markdown(
            f"### Priority {priority} — {risk_band}"
        )
    ...
    st.error(
        "Unable to load management recommendations."
    )
